...
async def run_server():
    ...
    if debug:
        print("Debug模式已开启", file=sys.stderr)

    # 读取配置文件中的所有数据库
    with open(args.config, 'r') as f:
        config = yaml.safe_load(f)
        if not config or 'databases' not in config:
            print("错误: 配置文件必须包含 databases 配置", file=sys.stderr)
            sys.exit(1)
    ...
    for db_name in databases:
        ...
        print(f"数据库 {db_name} 连接检测: {status_msg}", file=sys.stderr)
            ...
            for db_name in databases:
                if not connection_status[db_name]:
                    ...
                    if success:
                        ...
                        print(f"数据库 {db_name} 重新连接成功", file=sys.stderr)
                    else:
                        print(f"数据库 {db_name} 重试失败: {error}", file=sys.stderr)
